...
struct ad7879 {
	...
	bool			invert_x;
	bool			invert_y;
	bool			have_enable_gpio;
	int			enable_gpio;
	int			enable_gpio_en_value;
};
...
static int ad7879_parse_dt(struct device *dev, struct ad7879 *ts)
{
	...
	ts->enable_gpio = of_get_named_gpio_flags(dev->of_node,
		"enable-gpio", 0, &enable_gpio_flags);
        if (gpio_is_valid(ts->enable_gpio)) {
		ts->have_enable_gpio = 1;
		ts->enable_gpio_en_value =
			(enable_gpio_flags & OF_GPIO_ACTIVE_LOW) ? 0 : 1;

		err = devm_gpio_request(dev, ts->enable_gpio, "enable-gpio");
		if(err < 0) {
			dev_err(dev, "Request for enable-gpio failed!\n");
			ts->have_enable_gpio = 0;
		} else {
		        gpio_direction_output(ts->enable_gpio,
				ts->enable_gpio_en_value);
		}
	...
	ts->invert_x = device_property_read_bool(dev, "touchscreen-invert-x");
	ts->invert_y = device_property_read_bool(dev, "touchscreen-invert-y");
